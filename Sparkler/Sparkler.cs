...
//		private int fleet_id;
		...
		static void Main (string[] args) {
			...
			int ship_id = Convert.ToInt32(args[0]),
				run_times = Convert.ToInt32(args[1]);

//			fleet_id = Math.Abs(fleet_id);
			run_times = Math.Abs(run_times);

			Sparkler sparkler = new Sparkler(full_api_token, ship_id);
			sparkler.run(run_times);
		}

		public Sparkler (string full_api_token, int ship_id) {
...
//				}
				...
				runForOneShip(this.ship_id, run_times);
...
/*				if (morale >= 81) {
				...
				this.kcp.proxy(Map.START, Map.Start(1, /*this.fleet_id*/ 1, 1, 1));
